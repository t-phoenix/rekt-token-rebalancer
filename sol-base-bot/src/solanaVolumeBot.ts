import 'dotenv/config';
import { Connection, Keypair, PublicKey } from '@solana/web3.js';
import { getKeyPairFromPrivateKey } from './solana/utils.js';
import { PriceFetcher } from './utils/priceFetcher.js';
import { getUserConfig } from './utils/volumeBotConfig.js';
import { executeBuyTrade, executeSellTrade, getRandomTradeSize, shouldBuy } from './solana/pumpfun/volumebot/volumeBotTrading.js';
import { VolumeTracker, getWalletBalances } from './solana/pumpfun/volumebot/volumeBotTracking.js';
import { displayConfig, displaySummary, displayTradeHistory } from './solana/pumpfun/volumebot/volumeBotDisplay.js';

// ============================================================================
// MAIN BOT LOGIC
// ============================================================================

async function runVolumeBot() {
    console.clear();

    // 1. GET CONFIGURATION
    const config = await getUserConfig();

    // 2. INITIALIZE SOL PRICE FETCHER
    let solPriceUsd = 200; // Default fallback
    let priceFetcher: PriceFetcher | null = null;
    let lastPriceFetchTime = 0; // Track price fetch separately from summary

    const cmcApiKey = process.env.COINMARKETCAP_API_KEY;
    if (cmcApiKey) {
        try {
            priceFetcher = new PriceFetcher(cmcApiKey);
            solPriceUsd = await priceFetcher.getSolPrice();
            lastPriceFetchTime = Date.now(); // Initialize price fetch time
            console.log(`💰 Fetched live SOL price: $${solPriceUsd.toFixed(2)}\n`);
        } catch (error) {
            console.warn(`⚠️  Failed to fetch SOL price: ${error instanceof Error ? error.message : String(error)}`);
            console.warn(`   Using fallback price: $${solPriceUsd}\n`);
        }
    } else {
        console.warn('⚠️  COINMARKETCAP_API_KEY not set, using default SOL price: $200\n');
    }

    // 3. INITIALIZE CONNECTIONS AND WALLET
    const connection = new Connection(config.rpcUrl, 'confirmed');
    const wallet = getKeyPairFromPrivateKey(config.privateKey);
    const tokenMint = new PublicKey(config.tokenMint);

    // Display configuration
    displayConfig(config, solPriceUsd, priceFetcher);

    // 4. INITIALIZE TRACKING
    const tracker = new VolumeTracker();

    // Get initial balances
    const initialBalances = await getWalletBalances(connection, wallet, tokenMint);
    tracker.setInitialBalances(initialBalances.sol, initialBalances.token);

    console.log('💼 Initial Wallet State:');
    console.log(`   SOL: ${initialBalances.sol.toFixed(6)}`);
    console.log(`   Tokens: ${initialBalances.token.toFixed(2)}`);
    console.log(`   💵 Current SOL Price: $${solPriceUsd.toFixed(2)} ${priceFetcher ? '(live, updates every 5min)' : '(static)'}\n`);

    // 5. SET UP TRADING PARAMETERS
    const startTime = Date.now();
    const stopTime = config.runDurationMinutes > 0
        ? startTime + (config.runDurationMinutes * 60 * 1000)
        : null;

    let lastSummaryTime = Date.now();

    console.log('🚀 Starting Solana volume bot...\n');
    console.log('Press Ctrl+C to stop.\n');

    // 6. MAIN TRADING LOOP
    let isRunning = true;
    let totalVolumeUsd = 0;

    const tradingLoop = async () => {
        while (isRunning) {
            try {
                // Check stop conditions
                if (stopTime && Date.now() >= stopTime) {
                    console.log('\n⏰ Run duration reached. Stopping bot...');
                    isRunning = false;
                    break;
                }

                if (config.maxTotalVolumeUsd > 0 && totalVolumeUsd >= config.maxTotalVolumeUsd) {
                    console.log('\n💰 Max volume reached. Stopping bot...');
                    isRunning = false;
                    break;
                }

                // Update SOL price periodically (every 5 minutes)
                if (priceFetcher && Date.now() - lastPriceFetchTime >= 300000) {
                    try {
                        const newSolPrice = await priceFetcher.getSolPrice();
                        if (newSolPrice !== solPriceUsd) {
                            console.log(`\n🔄 SOL price updated: $${solPriceUsd.toFixed(2)} → $${newSolPrice.toFixed(2)}`);
                            solPriceUsd = newSolPrice;
                        }
                        lastPriceFetchTime = Date.now(); // Update price fetch time
                    } catch {
                        // Keep using last known price
                    }
                }

                // Determine trade direction
                const isBuy = shouldBuy(config.buyProbability);

                // Get current balances
                const currentBalances = await getWalletBalances(connection, wallet, tokenMint);

                // Execute trade
                let trade;

                if (isBuy) {
                    const tradeSize = getRandomTradeSize(
                        config.minTradeAmountSol,
                        config.maxTradeAmountSol,
                        config.randomizeTradeSize
                    );

                    // Check if we have enough SOL
                    if (currentBalances.sol < tradeSize + config.priorityFeeSol + 0.01) {
                        console.log('⚠️  Insufficient SOL balance for buy trade. Skipping...');
                        await new Promise(resolve => setTimeout(resolve, config.tradingIntervalMs));
                        continue;
                    }

                    trade = await executeBuyTrade(
                        connection,
                        wallet,
                        tokenMint,
                        tradeSize,
                        solPriceUsd,
                        config.priorityFeeSol,
                        config.slippagePercent
                    );
                } else {
                    // Sell a random percentage of token balance (10-50%)
                    const sellPercentage = 0.1 + Math.random() * 0.4;
                    const tokenAmount = currentBalances.token * sellPercentage;

                    // Check if we have enough tokens
                    if (currentBalances.token < 1) {
                        console.log('⚠️  Insufficient token balance for sell trade. Switching to buy...');

                        const tradeSize = getRandomTradeSize(
                            config.minTradeAmountSol,
                            config.maxTradeAmountSol,
                            config.randomizeTradeSize
                        );

                        trade = await executeBuyTrade(
                            connection,
                            wallet,
                            tokenMint,
                            tradeSize,
                            solPriceUsd,
                            config.priorityFeeSol,
                            config.slippagePercent
                        );
                    } else {
                        trade = await executeSellTrade(
                            connection,
                            wallet,
                            tokenMint,
                            tokenAmount,
                            solPriceUsd,
                            config.priorityFeeSol,
                            config.slippagePercent
                        );
                    }
                }

                // Record trade
                tracker.addTrade(trade);

                if (trade.success) {
                    totalVolumeUsd += trade.usdValue;
                }

                // Display summary if interval reached
                if (Date.now() - lastSummaryTime >= config.summaryIntervalMs) {
                    const currentBalances = await getWalletBalances(connection, wallet, tokenMint);
                    const pnl = tracker.calculatePnL(
                        currentBalances.sol,
                        currentBalances.token,
                        solPriceUsd
                    );

                    displaySummary(pnl);
                    displayTradeHistory(tracker.getTrades(), 5);

                    lastSummaryTime = Date.now();
                }

                // Wait for next trade
                await new Promise(resolve => setTimeout(resolve, config.tradingIntervalMs));
            } catch (error) {
                console.error('Error in trading loop:', error);
                await new Promise(resolve => setTimeout(resolve, config.tradingIntervalMs));
            }
        }
    };

    // 7. HANDLE GRACEFUL SHUTDOWN
    process.on('SIGINT', async () => {
        console.log('\n\n⏹️  Stopping bot...');
        isRunning = false;

        // Display final summary
        const finalBalances = await getWalletBalances(connection, wallet, tokenMint);
        const finalPnl = tracker.calculatePnL(
            finalBalances.sol,
            finalBalances.token,
            solPriceUsd
        );

        console.log('\n╔════════════════════════════════════════════════════════════╗');
        console.log('║                    FINAL SUMMARY                           ║');
        console.log('╚════════════════════════════════════════════════════════════╝');

        displaySummary(finalPnl);
        displayTradeHistory(tracker.getTrades(), 10);

        console.log('👋 Bot stopped. Goodbye!\n');
        process.exit(0);
    });

    // 8. START TRADING
    await tradingLoop();

    // Display final summary if bot stops naturally
    const finalBalances = await getWalletBalances(connection, wallet, tokenMint);
    const finalPnl = tracker.calculatePnL(
        finalBalances.sol,
        finalBalances.token,
        solPriceUsd
    );

    console.log('\n╔════════════════════════════════════════════════════════════╗');
    console.log('║                    FINAL SUMMARY                           ║');
    console.log('╚════════════════════════════════════════════════════════════╝');

    displaySummary(finalPnl);
    displayTradeHistory(tracker.getTrades(), 10);

    console.log('✅ Bot completed successfully!\n');
}

// ============================================================================
// ENTRY POINT
// ============================================================================

runVolumeBot().catch((error) => {
    console.error('Fatal error:', error);
    process.exit(1);
});
